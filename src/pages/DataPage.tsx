...
export default function DataPage() {
  ...
  return (
    <div>
      <header className="sticky top-0 z-10 bg-[#F5F7FA]/80 backdrop-blur-sm px-8 pt-6 pb-0 border-b border-slate-200/30">
        <h1 className="text-2xl font-bold font-[DM_Sans] text-slate-800 tracking-tight">
          Donnees
        </h1>
        <p className="text-sm text-slate-400 mt-0.5">
          Import CSV et exports Excel
        </p>
        ...
        <div className="flex gap-1 mt-4">
          {TABS.map(({ id, label, icon: Icon }) => (
            <button
              ...
              onClick={() => setTab(id)}
              className={`
                flex items-center gap-2 px-4 py-2 text-sm font-semibold font-[DM_Sans] rounded-t-xl transition-colors duration-150
                ${tab === id
                  ? 'bg-white text-primary-500 shadow-[0_1px_3px_rgba(0,0,0,0.08),0_1px_2px_rgba(0,0,0,0.06)]'
      ...
      <div className="px-8 pb-8 pt-6 space-y-6">
        {tab === 'import' ? <ImportContent /> : <ExportPanel />}
      </div>
    </div>
  );
}
          ...
          <Card>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                ...
                <StatCard label="Vivants" value={result.vivantsCount} colorClass="text-success-500" />
                <StatCard label="Termines" value={result.terminesCount} colorClass="text-slate-500" />
                <StatCard label="Ignores" value={result.skippedRows} colorClass="text-warning-500" />
              </div>

              <div className="flex items-center gap-2 text-sm text-slate-600">
                <Clock className="w-4 h-4" />
                Duree : <span className="font-medium">{formatDuration(result.parseDurationMs)}</span>
              </div>
                  ...
                  <ul className="space-y-1 max-h-40 overflow-y-auto text-sm text-slate-600 rounded-xl bg-slate-50 p-3">
                    {warnings.map((w, i) => (
                      <li key={i} className="flex gap-2">
                        <span className="text-slate-400 w-20 flex-shrink-0">Ligne {w.line}</span>
                        <span>{w.message}</span>
                      </li>
                    ))}
                  </ul>
